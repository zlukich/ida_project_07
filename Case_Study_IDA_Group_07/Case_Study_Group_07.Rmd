...
### 4.a Sales course 
The idea of this task it is to plot the sales course on specify locations of both brands OEM1 and OEM2.The type of graph used for this purpose is a line. 
From the image, it is possible to see, that the amount of cars sale over time is not constant but fluctuates.
```{r echo = FALSE}
knitr::include_graphics("Additional files/4.a.png")
```

```{r echo = FALSE}
knitr::include_graphics("Additional files/4.a3.png")
```

In case no location is specify, a message would be prompted to the user. 

```{r echo = FALSE}
knitr::include_graphics("Additional files/4.a2.png")
```


### 4.b.Defective 
In this task, the rate of defective vehicles gets calculated according to the different brands and location. For this task a barplot was used and the data was pivoted.
The graph shows that OEM1 has produced slightly less defective cars. On the other hand, at component and part level OEM2  has less quality Problems.

```{r echo = FALSE}
knitr::include_graphics("Additional files/4.b.png")
```
Similar to the graph above, the same effect appears here. OEM1 has less defective vehicles but OEM2 has less defective parts and components

```{r echo = FALSE}
knitr::include_graphics("Additional files/4.b2.png")
```
The user can also read the data from the table.

```{r echo = FALSE, out.width = "150%", fig.align = "center"}
knitr::include_graphics("Additional files/4.b3.png")
```
...
The screenshots of Forecast tab could be seen in the 5. Result part.
...
At the first date of 2014 linear model has for the vehicles sold calculated number of ~ 46 vehicles, during the last 2017 day calculated number reaches ~56. For the OEM2 picture looks different, calculated number of vehicles sold (by model) at the beginning is ~46, by the last day of Q1 2017 it falls to ~34. This indicates a positive sales trend for OEM1 and a negative sales trend for OEM2. Besides that, by looking at the defective vehicle rate from both brands, we can see that OEM1 has less defective rate compared to OEM2. Based on these facts, we recommend OEM1 as the more appropriate vehicle manufacturer.
