...
a.
```{r 1.a}
logistics_delay = inner_join(komponente_k7,logverzug_k7,by = "IDNummer",suffix = c(".log",".kom"))
...
logistics_delay = logistics_delay %>%
  mutate(Produktionsday = weekdays(Produktionsdatum,abbreviate = TRUE)) %>%
  mutate(Warenausgang  = ifelse(Produktionsday == "Fri" ,Produktionsdatum+3,Produktionsdatum)) %>%
  mutate(Warenausgang  = ifelse(Produktionsday == "Sat" ,Produktionsdatum+2,Produktionsdatum)) %>%
  mutate(Warenausgang  = ifelse(Produktionsday == "Sun" ,Produktionsdatum+1,Produktionsdatum+1)) %>%
  mutate(Warenausgang = as.Date(Warenausgang,origin = "1970-01-01",format = "%Y-%m-%d")) %>%
  mutate(Delay = Wareneingang - Warenausgang)
...
fig2 = plot_ly(logistics_delay,x = ~Delay,type = "histogram")
...
komponente_k2le2 = read.delim("Data/Komponente/Komponente_K2LE2.txt",sep = "\\") %>%
  select(X1,ID_Sitze) #%>% rename(ID_Sitze = Herstellernummer)

bestandteile_k2st2 = read_csv2("Data/Komponente/Bestandteile_Komponente_K2ST2.csv")
komponente_k2st2 = read_csv2("Data/Komponente/Komponente_K2ST2.csv") %>%
  select(X1,ID_Sitze)


#Typ 21
look_for_komponente_1 = read_csv2("Data/Fahrzeug/Bestandteile_Fahrzeuge_OEM2_Typ21.csv")
look_for_komponente_1 %>% filter(grepl("K2LE2",ID_Schaltung) | grepl("K2LE2",ID_Sitze) | grepl("K2LE2",ID_Motor))
look_for_komponente_1 %>% filter(grepl("K2ST2",ID_Schaltung) | grepl("K2ST2",ID_Sitze) | grepl("K2ST2",ID_Motor))

bestandteile_oem21 = look_for_komponente_1
#many entries :)
#Typ 22
look_for_komponente_2 = read_csv2("Data/Fahrzeug/Bestandteile_Fahrzeuge_OEM2_Typ22.csv")
look_for_komponente_2 %>% filter(grepl("K2LE2",ID_Schaltung) | grepl("K2LE2",ID_Sitze) | grepl("K2LE2",ID_Motor))
look_for_komponente_2 %>% filter(grepl("K2ST2",ID_Schaltung) | grepl("K2ST2",ID_Sitze) | grepl("K2ST2",ID_Motor))

bestandteile_oem22 = look_for_komponente_2
#many entries :)
# so we need to join Bestandteile of Fahrzeuge to Fahrzeuge und zulassungen, to get number of parts from Adelshofen
#Fahrzuege
fahrzeuge_typ21 = read_delim("Data/Fahrzeug/Fahrzeuge_OEM2_Typ21.csv",delim = ",")
fahrzeuge_typ22 = read_delim("Data/Fahrzeug/Fahrzeuge_OEM2_Typ22.csv")
