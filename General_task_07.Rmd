...
* 2. More Flexibility in terms of memory because each file can be store on different servers 
* 3. The user can do pick and choose, that means the user can decide which data they want to read and there is not need to read and load the whole table 
* 4. It is more secure. In one hand it would be possible to manage the access to the data therefore the users would get permissions to what they need and not to the whole data set. On the other hand,if an account gets hacked, the hacker would only see part of the information but the the whole data. 
