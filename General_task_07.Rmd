...
binwidth <-(14-1)/14
```
Since the calculated binwidth is 0,9, it would make much more sense to round it to 1, because the delay days are integer numbers.
...
<<<<<<< HEAD
* 2. We can manipulate with data separately for the cases when is isn't needed to import whole dataset, also there are possibility, that total table can be to huge to save it to memory. In our case f.e. to find number of registered parts we don't have to import parts tables, because the parts information already stored in bestandteile for components.
* 3. To avoid repeating of information and/or accumulation of NULL values. If tha data table consists of more that 1 data source or data essences, then it could be that 1 essence contains some attribute (in our case f.e. specific part) and other essence doesn't. That means all observations that doesn't contain that attribute will have a NULL value in that column (this can be massive). In our case if we create only 1 dataset containing all vehicles with their parts, some vehicles would have T16 part, but majority - not -> almost full column of NULL values
* 4. It is quicker to find information in separate file, because size of separate file is less than whole table
* 5. More Flexibility in terms of memory because each file can be store on different servers 
* 6. The user can do pick and choose, that means the user can decide which data they want to read and there is not need to read and load the whole table
Provided structure represent relational database structure. It contains reference keys, binding tables and built on table basis.
* 7. It is more secure. In one hand it would be possible to manage the access to the data therefore the users would get permissions to what they need and not to the whole data set. On the other hand,if an account gets hacked, the hacker would only see part of the information but the the whole data. 
The Database structure used in this case, is the relational. This means that the data points are related to one another. 


## Task 3 Components
To determine the number of cars that were build with T16 parts and got registered in Adelshofen, it is nessecary to know the car id of the cars that have  a T16 part.
...
#for semicolon delimited
T16_Kom_1 <- read_delim("Data/Komponente/Bestandteile_Komponente_K2LE2.csv",delim =";",show_col_types = FALSE)%>%dplyr::select("ID_T16","ID_K2LE2")
names(T16_Kom_1)[2]<-"ID_Sitze"
T16_Kom_2 <- read_delim("Data/Komponente/Bestandteile_Komponente_K2ST2.csv",delim =";",show_col_types = FALSE)%>%dplyr::select("ID_T16","ID_K2ST2")
